...
fromEncodedCbor (CTag 24 c) =
  ...
  in decodedToMaybe (withByteString f c)
    -- <|> decodedToMaybe (withByteStringLazy (f . B.toStrict) c)
fromEncodedCbor _ = Nothing

toEncodedCbor :: Cbor -> Maybe Cbor
toEncodedCbor = either (const Nothing) (pure . CTag 24 . CByteString . toStrict . toLazyByteString) . encode
...
fromEncodedURI (CTag 32 c) =
  ...
  in decodedToMaybe (withText f c) -- <|> decodedToMaybe (withTextlazy (f . TL.toStrict) c)
...
fromMIME (CTag 36 c) =
  decodedToMaybe (withText pure c) -- <|> decodedToMaybe (withTextLazy (pure . TL.toStrict) c)
fromMIME _ = Nothing
